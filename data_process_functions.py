...
idMap = pickle.load(open("rematch_to_id.p", "r"))
...
def dataprocessWithImage(SHEET_NAME,LIST_OF_BINARY_COLS,LIST_OF_REAL_COLS,ANSWER_COL,image_features_map):
    wbIn = openpyxl.load_workbook(SHEET_NAME)
    #MAX_ROW = wbIn.active.max_row
    wbOut = openpyxl.Workbook()
    columnCount = 0
    ...
    for i in range(2, maxrow+1):
        ...
        for j in range(1,maxcolumn):
            if j == ID_COL:
                rawID = str(sheet.cell(row=i,column=j).value)

                if i < 10:
                    print j, rawID

                if rawID in idMap:
#                    ID = idMap[str(sheet.cell(row=i,column=j).value)]
                    ID = rawID
                else:
#                    ID = -1
                    ID = rawID
...
#        print ID
        if ID in image_features_map:
            imageFeatures = image_features_map[ID]
            feats += imageFeatures

            data.append(feats)

            result.append(sheet.cell(row=i,column=maxcolumn).value)
#            print "Adding ID", ID, "to list of data points!"
